...
https://microsoftedge.microsoft.com/addons/detail/mmgfooecliddbadakcegfmjigjagllnh" target="_blank">Edge 商店安装</a>
      ...
      <div class="about-method method-3">
        ...
        <span class="badge-incomplete">功能不完整</span>
      </div>
    </div>
...
<div class="about-text small" style="margin-bottom: 6px;">如果这个小工具帮到了你，欢迎请我喝杯咖啡支持开发！</div>
    ...
    <div class="nopic-data-section" style="border-top: 1px solid rgba(255,255,255,0.06); margin-top: 12px; padding-top: 12px;">
      <div style="display:flex; gap:8px; justify-content:center;">
        <div id="nopic-export-data-btn" style="
            font-size: 12px;
            color: rgba(255,255,255,0.25);
            cursor: pointer;
            padding: 4px 12px;
        ...
        <div id="nopic-import-data-btn" style="
            font-size: 12px;
            color: rgba(255,255,255,0.25);
            cursor: pointer;
            padding: 4px 12px;
      ...
      <div id="nopic-reset-all-btn" style="
          font-size: 12px;
          color: rgba(255,255,255,0.25);
          cursor: pointer;
          padding: 6px 0 0 0;
...
`;
  ...
  const presetIcons = {
    ...
    github: (function () {
      ...
      const useDark = isLight || !isDarkMode;
      const color = useDark ? "%23333" : "%23fff";
      return `data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" fill="${color}"><path d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27s1.36.09 2 .27c1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/></svg>`;
    })(),
    learn:
...
`;
  ...
  document.addEventListener("click", function (e) {
    ...
    if (resetBtn) {
      ...
      showConfirmModal(
        "初始化当前网站数据",
        "将清除当前网站的所有设置、隐藏规则、图片控制、阅兵记录、自动点击器、文本替换、数据采集等数据。全局配置和隐私锁日志不会被清除。此操作不可撤销。",
        function () {
          var keysToRemove = [];
          ...
          Object.keys(_nopicStorageCache).forEach(function (k) {
            delete _nopicStorageCache[k];
          });
          _dcRestoring = true;
          showConfirmModal(
            "初始化完成",
            "当前网站数据已清除（全局配置和隐私锁日志已保留），刷新页面后生效。",
            function () {
              location.reload();
            },
            null,
    ...
    var exportBtn = e.target.closest("#nopic-export-data-btn");
    if (exportBtn) {
      e.stopPropagation();
      var siteHost = location.host;
      var siteUrlKey = encodeURIComponent(nopicGetUrlKey());
      var data = { _nopic_site: siteHost };
      for (var i = 0; i < localStorage.length; i++) {
        var k = localStorage.key(i);
        if (k && (k.indexOf(siteHost) !== -1 || k.indexOf(siteUrlKey) !== -1)) {
    ...
    var importBtn = e.target.closest("#nopic-import-data-btn");
    if (importBtn) {
      e.stopPropagation();
      var input = document.createElement("input");
      input.type = "file";
        ...
        reader.onload = function (ev) {
          try {
            ...
            Object.keys(imported).forEach(function (k) {
              if (k.indexOf(siteHost) !== -1 || k.indexOf(siteUrlKey) !== -1) {
