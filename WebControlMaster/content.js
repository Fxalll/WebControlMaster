(function () {
        ...
        if (items) {
          Object.keys(items).forEach(function(key) {
            if (key.startsWith('nopic_')) {
              ...
              try {
                localStorage.setItem('_nopic_ext_' + key, JSON.stringify(items[key]));
              } catch (e) {}
            }
          });
        }
  ...
  function nopicSetToggleState(prefix, enabled) {
    try {
      localStorage.setItem(nopicGetToggleKey(prefix), enabled ? 'true' : 'false');
    } catch (e) {}
  }
  ...
  function nopicGetConfigByPriority(prefix, defaultConfig, togglePrefix) {
    try {
      ...
      if (urlSaved) {
        ...
        // 开关状态从URL级别独立读取
        config.enabled = nopicGetToggleState(togglePrefix || prefix, defaultConfig.enabled);
  ...
  function getDisguiseConfig() {
    ...
    result.enabled = nopicGetToggleState('nopic_disguise', defaultDisguiseConfig.enabled);
    ...
    const levels = [
      () => {
        try {
          const s = localStorage.getItem('nopic_disguise_' + encodeURIComponent(nopicGetUrlKey()));
      ...
      () => {
        try {
          const s = localStorage.getItem('nopic_disguise_domain_' + location.host);
          return s ? JSON.parse(s) : null;
        } catch (e) {
          return null;
        }
    ...
    for (let i = 0; i < levels.length; i++) {
      const cfg = levels[i]();
      if (!cfg || cfg.titleMode === 'native' || cfg.titleMode === undefined) continue;
    ...
    for (let i = 0; i < levels.length; i++) {
      const cfg = levels[i]();
      if (!cfg || cfg.iconMode === 'native' || cfg.iconMode === undefined) continue;
  ...
  function setDisguiseConfig(cfg) {
    ...
    if (currentDisguiseScope === 'global') {
      ...
      const key = nopicGetConfigStorageKey('nopic_disguise', currentDisguiseScope);
  ...
  (function initDisguiseConfig() {
    if (currentDisguiseScope === 'global') {
      ...
      if (saved) disguiseConfig = typeof saved === 'string' ? JSON.parse(saved) : { ...saved };
    } else {
      const key = nopicGetConfigStorageKey('nopic_disguise', currentDisguiseScope);
      const saved = localStorage.getItem(key);
      if (saved) disguiseConfig = JSON.parse(saved);
    }
    disguiseConfig.enabled = nopicGetToggleState('nopic_disguise', defaultDisguiseConfig.enabled);
  })();
  ...
  function getMaskConfig() {
    ...
    config.enabled = nopicGetToggleState('nopic_mask', defaultMaskConfig.enabled);
...
function getEffectivePrivacyLock() {
  const levels = [
    ...
    },
    {
      ...
      get: () => {
        try {
          const s = localStorage.getItem('nopic_privacylock_domain_' + location.host);
          return s ? JSON.parse(s) : null;
        } catch (e) {
          return null;
        }
...
function hasValidConfig(cfg) {
    ...
    if (cfg.lockMode === 'manual' && (!cfg.password || cfg.password.length === 0)) {
        return false;
    }
...
function setPrivacyLockConfigByScope(scope, cfg) {
  ...
  // 检查是否是默认配置
  const isDefault = configToSave.lockMode === 'manual' && 
...
function addPrivacyLog(action, success) {
  ...
  localStorage.setItem('nopic_privacylog_' + location.host, JSON.stringify(logCfg));
}
  ...
  function getQuickTextConfigByScope(scope) {
    try {
      if (scope === 'global') {
        ...
        return saved ? (typeof saved === 'string' ? JSON.parse(saved) : saved) : { items: [] };
      }
      const key =
        scope === 'domain'
...
// 关闭按钮
document.getElementById('nopic-pagemonitor-close').addEventListener('click', e => {
  e.stopPropagation();
...
pageMonitorSubmenu.querySelectorAll('[data-pm-scope]').forEach(tab => {
  tab.addEventListener('click', e => {
    ...
    pageMonitorSubmenu.querySelectorAll('[data-pm-scope]').forEach(t => t.classList.remove('active'));
  ...
  // 拾取元素按钮
  document.getElementById('nopic-pm-pick-btn').addEventListener('click', e => {
    e.stopPropagation();
    if (isPickingElement) {
      stopElementPicking();
    } else {
...
function startElementPicking() {
  ...
  pickPreviewEl.style.cssText = 'position:fixed;z-index:2147483647;background:rgba(0,0,0,0.85);color:#fff;font-size:11px;padding:4px 8px;border-radius:4px;pointer-events:none;max-width:200px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;display:none;';
...
function stopElementPicking() {
  ...
  if (pickPreviewEl) { pickPreviewEl.remove(); pickPreviewEl = null; }
...
function onPickMouseMove(e) {
  ...
  // 过滤掉脚本UI元素
  if (el && (el.closest('#nopic-widget') || el.closest('#nopic-menu') || 
      el.closest("[id^='nopic-']") || el.classList.contains('nopic-pm-pick-overlay') ||
  ...
  if (!el || !el.tagName) {
    ...
    for (const element of elements) {
      if (!element.closest('#nopic-widget') && !element.closest('#nopic-menu') && 
          !element.closest("[id^='nopic-']") && !element.classList.contains('nopic-pm-pick-overlay') &&
  ...
  const text = (el.innerText || el.textContent || el.value || '').trim().substring(0, 50);
  pickPreviewEl.textContent = text || '[' + el.tagName.toLowerCase() + ']';
  pickPreviewEl.style.display = 'block';
  pickPreviewEl.style.left = Math.min(e.clientX + 15, window.innerWidth - 220) + 'px';
  pickPreviewEl.style.top = Math.min(e.clientY + 15, window.innerHeight - 50) + 'px';
}
...
function onPickClick(e) {
  ...
  const elementText = (el.innerText || el.textContent || el.value || '').trim().substring(0, 100);
  ...
  const newMonitor = {
    ...
    action: 'notification',
    name: (el.innerText || el.textContent || '').trim().substring(0, 50) || el.tagName.toLowerCase() + (el.id ? '#' + el.id : ''),
...
function generateSelector(el) {
  ...
  // 尝试使用 data-* 属性
  const dataAttrs = ['data-testid', 'data-id', 'data-qa', 'data-cy', 'data-test'];
  ...
  while (current && current !== document.body && depth < maxDepth) {
    ...
    if (current.className && typeof current.className === 'string') {
      ...
      if (classes.length > 0) {
        try {
          const classSel = '.' + classes.map(c => CSS.escape(c)).join('.');
          const fullSelector = path.length > 0 ? classSel + ' > ' + path.join(' > ') : classSel;
          // 检查这个选择器是否唯一
          const testSelector = path.length > 0 ? selector + classSel + ' > ' + path.join(' > ') : selector + classSel;
          // 简化：只检查当前的class是否唯一
...
function renderPageMonitorList() {
  ...
  if (monitors.length === 0) {
    container.innerHTML = '<div style="text-align:center;color:rgba(255,255,255,0.4);padding:20px;font-size:12px;">暂无监控规则，点击"添加"或"拾取"创建</div>';
    return;
  }
  ...
  container.querySelectorAll('[data-pm-field]').forEach(input => {
    input.addEventListener('change', e => {
      e.stopPropagation();
      updateMonitorField(e.target.dataset.pmId, e.target.dataset.pmField, e.target.value);
    });
  });
  ...
  // 绑定复选框
  container.querySelectorAll("[data-pm-field='stopAfterTrigger']").forEach(cb => {
...
function startAllPageMonitors() {
  ...
  allMonitors.forEach(m => { if (m.active) startMonitor(m.id); });
}
...
function startMonitor(monitorId) {
  ...
  const intervalId = setInterval(() => { checkMonitor(monitor); }, Math.max(100, monitor.checkInterval || 100));
...
function checkMonitor(monitor) {
  ...
  switch (monitor.rule) {
    case 'textChanged':
      // 只有当元素可见且文本变化时才触发
      if (currentVisible && lastValue !== undefined && currentText !== lastValue && currentText !== '') {
        triggered = true;
      ...
      break;
    case 'contains':
      if (currentVisible && currentText && monitor.keyword && currentText.includes(monitor.keyword)) {
        triggered = true;
        beforeValue = lastValue;
        afterValue = currentText;
      }
    ...
    case 'equals': {
      if (currentVisible) {
        ...
        if (!isNaN(num) && !isNaN(threshold)) {
          ...
          if (monitor.rule === 'equals' && Math.abs(num - threshold) < 0.001) {
            triggered = true;
            beforeValue = lastValue;
            afterValue = currentText;
          }
  ...
  if (wasVisible && !currentVisible) {
    triggered = true;
    // 获取之前保存的文本值
    const lastText = pageMonitorLastValues.get(monitor.id + '_text') || '未知内容';
...
function showElegantNotification(monitor, beforeValue, afterValue) {
  // 如果有旧弹窗，立即移除（让位给新弹窗）
  if (window.currentNotificationToast && window.currentNotificationToast.parentNode) {
  ...
  const beforeDisplay = beforeValue !== undefined && beforeValue !== null ? beforeValue : '无';
  const afterDisplay = afterValue !== undefined && afterValue !== null ? afterValue : '无';
...
function sendDesktopNotification(monitor, beforeValue, afterValue) {
  ...
  const notification = new Notification('🔔 页面监控触发', {
    ...
    icon: 'data:image/svg+xml;base64,'
  });
...
function executeMonitorAction(monitor, beforeValue, afterValue) {
  ...
  switch (monitor.action) {
    case 'refresh':
      ...
      break;
    case 'autoclicker':
      if (autoClickerConfig && autoClickerConfig.steps && autoClickerConfig.steps.length > 0) {
        executeAutoClicker();
      }
      break;
    case 'script':
      if (monitor.scriptCode) {
        try { (0, eval)(monitor.scriptCode); } catch(e) { console.error('脚本执行错误:', e); }
      }
      break;
    case 'desktop':  // 新增桌面通知
...
function getAutoClickerConfig() {
  const key = 'nopic_autoclicker_' + encodeURIComponent(location.origin + location.pathname);
  const saved = localStorage.getItem(key);
...
function setAutoClickerConfig(cfg) {
  localStorage.setItem(
    'nopic_autoclicker_' + encodeURIComponent(location.origin + location.pathname),
    JSON.stringify(cfg)
  );
}
  // 自动点击器安全窗口配置（按网页独立记忆）
  ...
  function getPageEditEnabled() {
    try {
      const saved = localStorage.getItem('nopic_pageedit_enabled_' + location.host);
      if (saved !== null) return saved === 'true';
    } catch (e) {}
    return true;
  }
  function setPageEditEnabled(enabled) {
    try {
      localStorage.setItem('nopic_pageedit_enabled_' + location.host, enabled ? 'true' : 'false');
    } catch (e) {}
  }
  ...
  function setParadeConfig(cfg) {
    localStorage.setItem('nopic_parade_config_' + location.host, JSON.stringify(cfg));
  }
 ...
 let loadAnimationConfig = getGlobalConfig('loadAnimation') !== undefined ? getGlobalConfig('loadAnimation') : defaultConfig.loadAnimation;
  ...
  document.addEventListener(
    ...
    e => {
      ...
      if (isRedLight) {
        ...
        for (const el of allEls) {
          if (el.classList.contains('nopic-clone') || el.classList.contains('nopic-parade-clone'))
            continue;
          const rect = el.getBoundingClientRect();
          const margin = 10;
          if (
      ...
      if (
        hitImage &&
        (window.imgHidenSet !== null || (isRedLight && effectiveZoomMode === 'middle'))
      ) {
        e.preventDefault();
        e.stopPropagation();
      }
...
function showElementDirectly(el) {
    ...
    function isElementInViewport(el) {
        ...
        return rect.bottom >= -200 && rect.top <= windowHeight + 200 &&
               rect.right >= -200 && rect.left <= windowWidth + 200;
    }

    // 判断元素是否需要动画（所有元素都需要，但会根据视窗决定动画还是直接显示）
    function shouldAnimateElement(el) {
        // 跳过脚本UI
        if (el.closest && (el.closest('#nopic-widget') || el.closest('#nopic-menu') || 
            el.closest("[id^='nopic-']") || el.closest('#nopic-zoom-container') ||
            el.closest('#nopic-parade-overlay'))) return false;
    ...
    function collectAllElements() {
        ...
        // 按从上到下排序
        toAnimate.sort((a, b) => a.getBoundingClientRect().top - b.getBoundingClientRect().top);
        return toAnimate;
    }
...
function groupElementsByRow(elements) {
    ...
    for (const el of sorted) {
        ...
        if (currentRow.length === 0 || Math.abs(top - currentRowTop) <= ROW_THRESHOLD) {
...
function processAllElements() {
    ...
    const observer = new MutationObserver((mutations) => {
        ...
        for (const mutation of mutations) {
            for (const node of mutation.addedNodes) {
                ...
                // 跳过脚本UI元素
                if (node.id === 'nopic-widget' || node.id === 'nopic-menu' || 
                    node.closest && (node.closest('#nopic-widget') || node.closest('#nopic-menu') || 
                    node.closest("[id^='nopic-']") || node.closest('#nopic-zoom-container') ||
                ...
                // 检查子元素
                const children = node.getElementsByTagName ? node.getElementsByTagName('*') : [];
        ...
        if (newElements.length > 0) {
            ...
            // 新元素按位置排序
            newElements.sort((a, b) => a.getBoundingClientRect().top - b.getBoundingClientRect().top);
    ...
    window.addEventListener('scroll', function() {
        ...
        const remaining = allElements.filter(el => el.dataset.nopicShown === 'false');
  ...
  <button class="nopic-privacy-btn primary" id="nopic-autoclicker-save-execute-btn" style="flex:1;padding:6px 0;font-size:12px;border-radius:4px;background:rgba(74,222,128,0.25);border-color:rgba(74,222,128,0.5);color:#4ade80;">保存并执行</button>
    ...
    <div style="margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,255,255,0.1);">
      <div style="font-size:11px;color:rgba(255,255,255,0.5);line-height:1.5;">
        <div>• 点击"添加点击"后，3秒内点击页面元素即可记录</div>
        <div>• 延时单位为毫秒（1000ms = 1秒）</div>
        <div>• 保存后刷新页面会自动执行整套流程</div>
      </div>
    </div>
  `;
...
function showAutoClickerStatusPanel() {
  ...
  if (textEl) {
    textEl.style.display = autoClickerConfig.showNotification === false ? 'none' : '';
  }
}
...
document.getElementById('nopic-ac-status-stop').addEventListener('click', () => {
  ...
  setTimeout(() => {
    ...
    const notifToggle = document.getElementById('nopic-autoclicker-notif-toggle');
    if (notifToggle) {
      if (autoClickerConfig.showNotification === false) notifToggle.classList.remove('on');
      else notifToggle.classList.add('on');
    }
    const autostartToggle = document.getElementById('nopic-autoclicker-autostart-toggle');
    if (autostartToggle) {
      if (autoClickerConfig.autoStartOnLoad === false) autostartToggle.classList.remove('on');
  ...
  textReplaceSubmenu.innerHTML = `
    ...
    <div style="font-size:11px;color:rgba(255,255,255,0.5);margin-bottom:8px;">后面的覆盖前面的同名规则。规则自动生效，无需开关。</div>
...
function showPinInput() {
  ...
  wrapper.style.transition = 'transform 0.4s cubic-bezier(0.34, 1.0, 0.64, 1)';
  wrapper.style.transform = 'translateY(-8%)';
  
  pinInputState.value = '';
  pinInputState.isActive = true;
  pinInputState.attempts = 0;
  document.querySelectorAll('.nopic-lock-pin-dot').forEach(d => d.className = 'nopic-lock-pin-dot');
...
var canSkip = (savedState !== 'false');
  ...
  dots.forEach(function(dot, i) {
    dot.className = 'nopic-lock-pin-dot' + (i < pinInputState.value.length ? ' filled' : '');
  });
  ...
  if (pinInputState.value.length === 4) {
    ...
    if (pinInputState.value === effective.password) {
      ...
      pinInputState.value = '';
      dots.forEach(function(d) { d.className = 'nopic-lock-pin-dot error'; });
...
skipEl.onclick = function(e) {
  ...
  showConfirmModal(
    ...
    function() {
      ...
      // 设置待重置标志，用于刷新后提醒
      localStorage.setItem('nopic_pending_password_reset_' + location.host, 'true');
      window._nopicResetAcknowledged = false;
      // 先解锁
      hidePrivacyLockOverlay(true);
    ...
    function() {
      ...
      pinInputState.value = '';
      pinInputState.attempts = 0;
      document.querySelectorAll('.nopic-lock-pin-dot').forEach(function(d) {
...
`;
  ...
  function updateAllUI() {
    ...
    if (paradeMenuEntry) paradeMenuEntry.style.display = paradeConfig.enabled ? 'flex' : 'none';
    ...
    const tabDisguiseMenuEntry = document.getElementById('nopic-tabdisguise-menu-entry');
    if (disguiseToggle) disguiseToggle.classList.toggle('on', disguiseConfig.enabled);
    if (tabDisguiseMenuEntry)
      tabDisguiseMenuEntry.style.display = disguiseConfig.enabled ? 'flex' : 'none';
    // 更新场景按钮状态（显示配置实际来自哪个级别）
    tabDisguiseSubmenu.querySelectorAll('[data-scope]').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.scope === currentDisguiseScope);
    });
    ...
    if (maskMenuEntry) maskMenuEntry.style.display = maskConfig.enabled ? 'flex' : 'none';
    // 更新场景按钮状态
    ...
    // ===== 网页隐私锁UI更新 =====
    const privacyLockToggle = document.getElementById('nopic-privacylock-toggle');
    const privacyLockMenuEntry = document.getElementById('nopic-privacylock-menu-entry');
    if (privacyLockToggle) privacyLockToggle.classList.toggle('on', privacyLockConfig.enabled);
    if (privacyLockMenuEntry)
      privacyLockMenuEntry.style.display = privacyLockConfig.enabled ? 'flex' : 'none';
...
['url', 'domain', 'global'].forEach(scope => {
  ...
  if (activeTag) {
    ...
    if (textReplaceToggle)
      textReplaceToggle.classList.toggle('on', nopicGetToggleState('nopic_textreplace', true));
    const textReplaceMenuEntry = document.getElementById('nopic-textreplace-menu-entry');
    const isTrActive = nopicGetToggleState('nopic_textreplace', true); // 读取独立记忆
    if (textReplaceMenuEntry) textReplaceMenuEntry.style.display = isTrActive ? 'flex' : 'none';
    // 更新场景按钮状态
    textReplaceSubmenu.querySelectorAll('[data-scope]').forEach(btn => {
      btn.classList.toggle('active', btn.dataset.scope === currentTextReplaceScope);
    });
    // 更新替换规则列表
    ...
    const autoClickerMenuEntry = document.getElementById('nopic-autoclicker-menu-entry');
    if (autoClickerToggle) autoClickerToggle.classList.toggle('on', autoClickerConfig.enabled);
    if (autoClickerMenuEntry)
      autoClickerMenuEntry.style.display = autoClickerConfig.enabled ? 'flex' : 'none';

    // ===== 页面监控器UI更新 =====
    const pageMonitorToggle = document.getElementById('nopic-pagemonitor-toggle');
    const pageMonitorMenuEntry = document.getElementById('nopic-pagemonitor-menu-entry');
    ...
    const quickTextMenuEntry = document.getElementById('nopic-quicktext-menu-entry');
    if (quickTextToggle) quickTextToggle.classList.toggle('on', quickTextEnabled);
    if (quickTextMenuEntry) quickTextMenuEntry.style.display = quickTextEnabled ? 'flex' : 'none';

    // ===== 页面可编辑UI更新 =====
    const pageEditToggle = document.getElementById('nopic-pageedit-toggle');
    const pageEditMenuEntry = document.getElementById('nopic-pageedit-menu-entry');
    if (pageEditToggle) pageEditToggle.classList.toggle('on', pageEditEnabled);
    if (pageEditMenuEntry) pageEditMenuEntry.style.display = pageEditEnabled ? 'flex' : 'none';
    ...
    const col1Visible = paradeConfig.enabled || textReplaceConfig.enabled || pageEditEnabled;
    const col2Visible = maskConfig.enabled || disguiseConfig.enabled || privacyLockConfig.enabled;
    const col3Visible = quickTextEnabled || autoClickerConfig.enabled || pageMonitorEnabled;
    ...
    tabDisguiseSubmenu.querySelectorAll('[data-title-mode]').forEach(row => {
      row
        ...
        .classList.toggle('checked', row.dataset.titleMode === disguiseConfig.titleMode);
    });
    ...
    const presetTitleSelect = document.getElementById('nopic-preset-title-select');
    if (titleInput) {
      titleInput.value = disguiseConfig.customTitle || '';
      titleInput.style.display = disguiseConfig.titleMode === 'custom' ? 'block' : 'none';
    }
    if (presetTitleRow) {
      presetTitleRow.style.display = disguiseConfig.titleMode === 'preset' ? 'block' : 'none';
    }
    if (presetTitleSelect) {
      const header = presetTitleSelect.querySelector('.nopic-custom-select-header');
      const options = presetTitleSelect.querySelectorAll('.nopic-custom-select-option');
      // 更新选中状态
      options.forEach(opt => {
        opt.classList.toggle('selected', opt.dataset.value === disguiseConfig.presetTitle);
    ...
    tabDisguiseSubmenu.querySelectorAll('[data-icon-mode]').forEach(row => {
      row
        .querySelector('.nopic-radio')
        .classList.toggle('checked', row.dataset.iconMode === disguiseConfig.iconMode);
    });
    ...
    if (iconUploadArea)
      iconUploadArea.style.display = disguiseConfig.iconMode === 'custom' ? 'flex' : 'none';
    if (presetIconRow)
      presetIconRow.style.display = disguiseConfig.iconMode === 'preset' ? 'block' : 'none';
    if (presetIconSelect) {
      const header = presetIconSelect.querySelector('.nopic-custom-select-header');
      const options = presetIconSelect.querySelectorAll('.nopic-custom-select-option');
      // 更新选中状态
      options.forEach(opt => {
        opt.classList.toggle('selected', opt.dataset.value === disguiseConfig.presetIcon);
...
document.querySelectorAll('.nopic-privacy-pin-input').forEach(input => {
  ...
  input.addEventListener('blur', function(e) {
    ...
    const tip = parent.querySelector('.nopic-pin-tip');
    if (tip) {
      tip.textContent = this.value.length === 4 ? '已设置' : '点击输入框设置4位PIN';
...
if (document.getElementById('nopic-disguise-list-url')) {
  ...
  document.getElementById('nopic-privacylock-toggle').addEventListener('click', e => {
    e.stopPropagation();
...
function showConfirmModalWithAck(title, text) {
  ...
  document.getElementById('nopic-disguise-toggle').addEventListener('click', e => {
    e.stopPropagation();
  ...
  tabDisguiseSubmenu.querySelectorAll('[data-title-mode]').forEach(row => {
    row.addEventListener('click', e => {
      ...
      if (presetTitleRow)
        presetTitleRow.style.display = disguiseConfig.titleMode === 'preset' ? 'block' : 'none';
      if (customTitleInput)
        customTitleInput.style.display = disguiseConfig.titleMode === 'custom' ? 'block' : 'none';
      updateAllUI();
      applyDisguise();
    });
  });
  ...
  if (presetTitleSelect) {
    const header = presetTitleSelect.querySelector('.nopic-custom-select-header');
    const dropdown = presetTitleSelect.querySelector('.nopic-custom-select-dropdown');
    const options = presetTitleSelect.querySelectorAll('.nopic-custom-select-option');
  ...
  tabDisguiseSubmenu.querySelectorAll('[data-icon-mode]').forEach(row => {
    row.addEventListener('click', e => {
      ...
      if (presetIconRow)
        presetIconRow.style.display = disguiseConfig.iconMode === 'preset' ? 'block' : 'none';
      if (customIconArea)
        customIconArea.style.display = disguiseConfig.iconMode === 'custom' ? 'flex' : 'none';
      updateAllUI();
      applyDisguise();
    });
  });
  ...
  if (presetIconSelect) {
    const header = presetIconSelect.querySelector('.nopic-custom-select-header');
    const dropdown = presetIconSelect.querySelector('.nopic-custom-select-dropdown');
    const options = presetIconSelect.querySelectorAll('.nopic-custom-select-option');
  ...
  // 图标上传
  document.getElementById('nopic-icon-upload-btn').addEventListener('click', e => {
    e.stopPropagation();
    document.getElementById('nopic-icon-file-input').click();
  });

  document.getElementById('nopic-icon-file-input').addEventListener('change', async e => {
  ...
  // 清空自定义图标
  document.getElementById('nopic-clear-icon-btn').addEventListener('click', e => {
    e.stopPropagation();
  ...
  // 重置按钮
  document.getElementById('nopic-disguise-reset-btn').addEventListener('click', e => {
    e.stopPropagation();
  ...
  tabDisguiseSubmenu.querySelectorAll('[data-scope]').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      if (newScope === 'global') {
        ...
        const saved = localStorage.getItem(key);
        disguiseConfig = saved ? JSON.parse(saved) : { ...defaultDisguiseConfig };
      }
      disguiseConfig.enabled = nopicGetToggleState('nopic_disguise', defaultDisguiseConfig.enabled);
      updateAllUI();
    });
...
function getDisguiseConfigByScope(scope) {
  ...
  scopes.forEach((scope, idx) => {
    ...
    const hasData = cfg.titleMode && cfg.titleMode !== 'native' || cfg.iconMode && cfg.iconMode !== 'native';
...
const hasUrlData = !!(urlCfg.titleMode && urlCfg.titleMode !== 'native' || urlCfg.iconMode && urlCfg.iconMode !== 'native');
const hasDomainData = !!(domainCfg.titleMode && domainCfg.titleMode !== 'native' || domainCfg.iconMode && domainCfg.iconMode !== 'native');
const hasGlobalData = !!(globalCfg.titleMode && globalCfg.titleMode !== 'native' || globalCfg.iconMode && globalCfg.iconMode !== 'native');
...
const borderColor = isEffective ? 'rgba(96,165,250,0.3)' : 'rgba(255,255,255,0.05)';
...
`;
  ...
  document.querySelectorAll('[data-disguise-edit]').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      disguiseConfig.enabled = nopicGetToggleState('nopic_disguise', defaultDisguiseConfig.enabled);
      // 关闭当前三列视图，显示编辑界面
      document.getElementById('nopic-disguise-list-view')?.style.setProperty('display', 'none');
      document.getElementById('nopic-disguise-edit-view')?.style.setProperty('display', 'flex');
      updateAllUI();
      applyDisguise();
    });
  });
  ...
  function observeTitle() {
    ...
    titleObserver = new MutationObserver(() => {
      ...
      let target = null;
      if (effective.titleMode === 'custom' && effective.customTitle) target = effective.customTitle;
  ...
  function applyTextReplaceIncremental() {
    ...
    while ((node = walker.nextNode())) {
      ...
      if (!parent) continue;
      // 跳过已经在替换 span 里的文本节点
      if (parent.classList && parent.classList.contains('nopic-textreplace-span')) continue;
      if (
    ...
    textNodes.forEach(textNode => {
      ...
      if (hasReplaced && matchedRule) {
        ...
        const afterText = originalText.slice(matchIndex + matchedRule.from.length);
  ...
  function createMaskLayer(maskData) {
    ...
    layer.style.borderRadius = (maskData.radius != null ? maskData.radius : 8) + 'px';
  ...
  function startMaskDraw() {
    ...
    maskDrawCurrentOpacity = parseInt(document.getElementById('nopic-mask-opacity').value);
    const activePosBtn = document.querySelector('.nopic-mask-position-btn.active');
    maskDrawCurrentPosition = activePosBtn ? activePosBtn.dataset.position : 'absolute';
...
function endMaskDraw() {
  ...
  function updateMaskList() {
    ...
    const totalCount = urlMasks.length + domainMasks.length + globalMasks.length;
    ...
    function renderList(container, masks, scope) {
      ...
      masks.forEach(mask => {
        ...
        // 点击删除
        item.querySelector('.nopic-mask-item-delete').addEventListener('click', e => {
          e.stopPropagation();
  ...
  // 遮罩开关事件
  // 阅兵模式开关
  document.getElementById('nopic-parade-toggle').addEventListener('click', e => {
    e.stopPropagation();
  ...
  document.getElementById('nopic-mask-toggle').addEventListener('click', e => {
    e.stopPropagation();
  ...
  // 透明度滑块事件
  document.getElementById('nopic-mask-opacity').addEventListener('input', e => {
    const val = e.target.value;
    document.getElementById('nopic-mask-opacity-val').textContent = val + '%';
    maskDrawCurrentOpacity = parseInt(val);
  });
  // 圆角滑块事件
  document.getElementById('nopic-mask-radius').addEventListener('input', e => {
    const val = e.target.value;
  ...
  // 阻止颜色选择器和滑块的冒泡
  document.getElementById('nopic-mask-color').addEventListener('mousedown', e => {
    e.stopPropagation();
  });
  document.getElementById('nopic-mask-opacity').addEventListener('mousedown', e => {
    e.stopPropagation();
  });
  ...
  // 开始绘制按钮
  document.getElementById('nopic-mask-draw-btn').addEventListener('click', e => {
    e.stopPropagation();
    if (isDrawingMask) {
      endMaskDraw();
    } else {
  ...
  // 清除全部按钮
  document.getElementById('nopic-mask-clear-btn').addEventListener('click', e => {
    e.stopPropagation();
    clearAllMasks();
  });
  ...
  const hideTabDisguiseSubmenu = () => {
    ...
    tabDisguiseSubmenu.classList.remove('active'); setTimeout(() => { tabDisguiseSubmenu.style.display = 'none'; }, 250);
  };
...
if (disguiseBackBtn) {
  disguiseBackBtn.addEventListener('click', e => {
    e.stopPropagation();
    document.getElementById('nopic-disguise-list-view').style.display = 'flex';
    document.getElementById('nopic-disguise-edit-view').style.display = 'none';
    renderDisguiseLists();
    applyDisguise();
  });
}
  ...
  // 关闭按钮事件
  const tabDisguiseCloseBtn = document.getElementById('nopic-tabdisguise-close');
  ...
  if (timeFormatInput) timeFormatInput.value = appConfig.timeFormat || '{时}:{分}';
  ...
  // 关闭按钮事件
  const privacyLockCloseBtn = document.getElementById('nopic-privacylock-close');
  ...
  // ===== 替换文字子菜单（弹窗方式） =====
  const textReplaceTrigger = document.getElementById('nopic-textreplace-menu-entry');
  ...
  // 关闭按钮事件
  const textReplaceCloseBtn = document.getElementById('nopic-textreplace-close');
  ...
  // ===== 自动点击器子菜单（弹窗方式） =====
  const autoClickerTrigger = document.getElementById('nopic-autoclicker-menu-entry');
  ...
  const autoClickerCloseBtn = document.getElementById('nopic-autoclicker-close');
  ...
  const showQuickTextSubmenu = () => {
    ...
    showPopupAtTrigger(quickTextSubmenu, document.getElementById('nopic-quicktext-menu-entry'));
  ...
  document.getElementById('nopic-quicktext-close').addEventListener('click', e => {
    e.stopPropagation();
    hideQuickTextSubmenu();
  });

  document.getElementById('nopic-quicktext-menu-entry').addEventListener('click', e => {
    e.stopPropagation();
  ...
  document.getElementById('nopic-quicktext-toggle').addEventListener('click', e => {
    e.stopPropagation();
  ...
  // 抽离出的行内编辑逻辑
  function enterEditMode(containerDiv, scope, idx, itemData, focusRecord = false) {
    ...
    containerDiv.querySelector('.nopic-qt-display-shortcut').style.display = 'none';
  ...
  quickTextSubmenu.querySelectorAll('[data-qt-add]').forEach(btn => {
    btn.addEventListener('click', e => {
      ...
      const textInput = editRow.querySelector("input[type='text']:not(.nopic-qt-shortcut-input)");
      const shortcutInput = editRow.querySelector('.nopic-qt-shortcut-input');
      const recordBtn = editRow.querySelector('.nopic-qt-record-btn');
      const saveBtn = editRow.querySelector('.nopic-qt-save-btn');
      const cancelBtn = editRow.querySelector('.nopic-qt-cancel-btn');
  ...
  menu.addEventListener('mouseleave', () => {
    setTimeout(() => {
      ...
      if (maskColorInput && document.activeElement === maskColorInput) isInputFocused = true;
  ...
  disguiseSubmenu.addEventListener('mouseleave', () => {
    setTimeout(() => {
      ...
      if (!disguiseSubmenu.matches(':hover') && !disguiseTrigger.matches(':hover')) {
        hideDisguiseSubmenu();
      }
    }, 100);
  });
...
if (privacyLockRuntime.isLocked && pinInputState.isActive) {
  ...
  if (e.key === 'Escape') {
    ...
    const hint = document.getElementById('nopic-lock-hint');
    const arrow = document.querySelector('.nopic-privacy-lock-arrow');
    if (hint) { hint.style.opacity = '1'; hint.textContent = '上拉以解锁'; }
    if (arrow) arrow.style.opacity = '1';
    wrapper.style.transition = 'transform 0.5s cubic-bezier(0.34, 1.0, 0.64, 1)';
...
function updateLockTime() {
  ...
  // 更新时间文字
  const formatted = formatLockTime(appearance.timeFormat, appearance.customText);
...
function initLockDrag() {
  ...
  wrapper.addEventListener('touchstart', onLockDragStartTouch, { passive: true });
  document.addEventListener('mousemove', onLockDragMove);
  document.addEventListener('mouseup', onLockDragEnd);
  document.addEventListener('touchmove', onLockDragMoveTouch, { passive: false });
...
bg.style.background = app.bgColor;
  ...
  updateLockTime();
  if (privacyLockRuntime._clockInterval) clearInterval(privacyLockRuntime._clockInterval);
  privacyLockRuntime._clockInterval = setInterval(updateLockTime, 1000);
  
  requestAnimationFrame(() => {
    wrapper.style.transition = 'transform 0.7s cubic-bezier(0.34, 1.0, 0.64, 1)';
    wrapper.style.transform = 'translateY(0)';
  });
...
function attemptPrivacyUnlock(password) {
  ...
  // 检查密码是否匹配
  if (password === effective.password && effective.password && effective.password.length === 4) {
    // 密码解锁成功 - 清除所有待重置标志
    localStorage.removeItem('nopic_pending_password_reset_' + location.host);
    window._nopicResetAcknowledged = false;
  ...
  function stopPrivacyLockListeners() {
    ...
    if (window._nopicPrivacyLockThrottledUpdate) {
      ['mousemove', 'keydown', 'click', 'scroll'].forEach(event => {
        document.removeEventListener(event, window._nopicPrivacyLockThrottledUpdate);
      });
  ...
  if (unlockBtn) {
    unlockBtn.addEventListener('click', () => {
      const unlockPasswordInput = document.getElementById('nopic-privacy-unlock-password');
...
function initPrivacyLock() {
  // 检测待重置密码标志
  var pendingReset = localStorage.getItem('nopic_pending_password_reset_' + location.host);
  ...
  if (pendingReset === 'true') {
    ...
    setTimeout(function() {
      ...
      // 修改确认按钮样式
      var confirmBtn = document.querySelector('#nopic-confirm-modal .nopic-confirm-btn.danger');
      if (confirmBtn) {
        confirmBtn.textContent = '我知道了';
        confirmBtn.classList.remove('danger');
        confirmBtn.classList.add('cancel');
      }
      var cancelBtn = document.querySelector('#nopic-confirm-modal .nopic-confirm-btn.cancel:not(.danger)');
...
const typeLabels = {
  ...
  const targetInfo = step.targetSelector || step.targetXpath || '等待下一步元素';
  const shortInfo = targetInfo.length > 25 ? targetInfo.substring(0, 25) + '...' : targetInfo;
  detailHtml = `<div style="font-size:11px;color:rgba(255,255,255,0.5);display:flex;align-items:center;gap:6px;">
    <span>等待:</span>
    ...
    <span style="font-size:10px;color:rgba(255,255,255,0.3);">(${step.timeout || 30}s超时)</span>
        ...
        const shortText = selectorText.length > 30 ? selectorText.substring(0, 30) + '...' : selectorText;
        ...
        const textShort = step.text && step.text.length > 20 ? step.text.substring(0, 20) + '...' : (step.text || '');
        ...
        if (step.selector || step.xpath) {
          const selShort = (step.selector || step.xpath || '').substring(0, 25) + '...';
        ...
        const codeShort = step.code && step.code.length > 30 ? step.code.substring(0, 30) + '...' : (step.code || '');
...
function addWaitStep() {
  ...
  const textEl = document.getElementById('nopic-ac-status-text');
  if (textEl) {
    const linkedInfo = waitStep.targetSelector || waitStep.targetXpath || '等待下一步元素';
...
function autoLinkWaitStep(waitStep) {
  ...
  for (let i = waitIndex + 1; i < steps.length; i++) {
    const s = steps[i];
    if ((s.type === 'click' || s.type === 'input') && (s.selector || s.xpath)) {
  ...
  function addClickStep() {
    ...
    const clickHandler = e => {
      ...
      function getUniqueSelector(targetEl) {
        if (targetEl.id && document.querySelectorAll('#' + CSS.escape(targetEl.id)).length === 1) {
      ...
      if (!selector) {
        ...
        if (directTexts.length > 0) {
          ...
          const escapedTxt =
            txt.includes('"') && txt.includes("'") ? txt.replace(/"/g, '&quot;') : txt;
          const quote = escapedTxt.includes('"') ? "'" : '"';
          parts[parts.length - 1] += `[contains(text(), ${quote}${escapedTxt}${quote})]`;
          xpath = '/' + parts.join('/');
        }
      } else {
        xpath = getStableXPath(el);
...
function addPositionStep() {
  ...
  const clickHandler = e => {
    if (!autoClickerRecordingPosition) return;
    if (e.target.closest('#nopic-autoclicker-submenu') || 
        e.target.closest('#nopic-widget') || e.target.closest('#nopic-menu')) {
      return;
    }
    ...
    const tip = document.getElementById('nopic-autoclicker-record-position-tip');
    if (tip) tip.remove();
    document.removeEventListener('click', clickHandler, true);
    updateAutoClickerFlowList();
  };
...
dialog.addEventListener('click', function(e) {
  ...
  const ell = e.target.closest('input, textarea, [contenteditable="true"], [contenteditable=""], [role="textbox"], [role="searchbox"]');
  if (!ell) {
    // 点击的不是可输入元素，忽略
    return;
  }
      ...
      const tip = document.getElementById('nopic-autoclicker-record-input-tip');
      if (tip) tip.remove();
      document.removeEventListener('click', clickHandler, true);
      updateAutoClickerFlowList();
    };
...
dialog.addEventListener('click', function(e) {
  ...
  const confirmBtn = dialog.querySelector('#nopic-autoclicker-script-confirm');
...
function executeAutoClicker() {
  if (!autoClickerConfig.enabled || autoClickerConfig.steps.length === 0) return;
  ...
  if (!targetSelector && !targetXpath) {
    if (!isTerminated) {
      updateAutoClickerStatus(`第 ${currentLoop} 轮 - 无目标元素，跳过等待`, 'pending');
    }
    setTimeout(() => { if (!isTerminated) runNext(); }, 200);
    return;
  }
  ...
  function checkElement() {
    ...
    if (isVisible) {
      // 元素已加载
      if (!isTerminated && autoClickerConfig.showNotification) {
        updateAutoClickerStatus(`第 ${currentLoop} 轮 - 目标元素已加载`, 'success');
      }
      // 视觉反馈
      try {
    ...
    if (Date.now() - startTime > waitTimeout) {
      if (!isTerminated) {
        updateAutoClickerStatus(`第 ${currentLoop} 轮 - 等待超时，未找到目标元素`, 'error');
      }
      setTimeout(() => { if (!isTerminated) runNext(); }, 300);
      return;
    }
        ...
        if (target) {
          ...
          if (!isTerminated) {
            if (autoClickerConfig.showNotification) {
              updateAutoClickerStatus(`第 ${currentLoop} 轮 - 步骤 ${stepNum} 点击成功`, 'success');
            } else {
              updateAutoClickerStatus(
  ...
  if (!isTerminated && autoClickerConfig.showNotification) {
    updateAutoClickerStatus(`第 ${currentLoop} 轮 - 位置点击成功 (${Math.round(x)}, ${Math.round(y)})`, 'success');
  }
              
              // 视觉反馈 - 圆圈扩散
              const flash = document.createElement('div');
              flash.style.cssText =
                'position:fixed;pointer-events:none;z-index:2147483646;border:2px solid #60a5fa;border-radius:50%;width:20px;height:20px;transform:translate(-50%,-50%);transition:all 0.3s;';
              ...
              if (!isTerminated) {
                updateAutoClickerStatus(`第 ${currentLoop} 轮 - 位置点击失败：无元素`, 'error');
              }
            }
            setTimeout(() => { if (!isTerminated) runNext(); }, 300);
          }, 400);
          ...
          if (!isTerminated) {
            updateAutoClickerStatus(`第 ${currentLoop} 轮 - 位置点击错误`, 'error');
          }
          setTimeout(() => { if (!isTerminated) runNext(); }, 300);
        }
        ...
        if (target) {
          try {
            ...
            setTimeout(() => {
              ...
              if (!isTerminated && autoClickerConfig.showNotification) {
                updateAutoClickerStatus(`第 ${currentLoop} 轮 - 输入成功`, 'success');
              }
            ...
            if (!isTerminated) {
              updateAutoClickerStatus(`第 ${currentLoop} 轮 - 输入失败`, 'error');
            }
            setTimeout(() => { if (!isTerminated) runNext(); }, 300);
          }
        } else {
          if (!isTerminated) {
            updateAutoClickerStatus(`第 ${currentLoop} 轮 - 输入失败：未找到输入框`, 'error');
          }
          setTimeout(() => { if (!isTerminated) runNext(); }, 300);
        }
        ...
        try {
          ...
          if (!isTerminated) {
            updateAutoClickerStatus(`第 ${currentLoop} 轮 - 脚本执行成功`, 'success');
          }
        } catch (e) {
          if (!isTerminated) {
            updateAutoClickerStatus(`第 ${currentLoop} 轮 - 脚本执行错误: ${e.message}`, 'error');
          }
        }
        setTimeout(() => { if (!isTerminated) runNext(); }, 300);
      }
  ...
  const autoClickerAddDelay = document.getElementById('nopic-autoclicker-add-delay');
  ...
  const autoClickerAddClick = document.getElementById('nopic-autoclicker-add-click');
  ...
  // 智能等待按钮
const autoClickerAddWait = document.getElementById('nopic-autoclicker-add-wait');
if (autoClickerAddWait) {
  ...
  // 添加点击位置按钮
const autoClickerAddPosition = document.getElementById('nopic-autoclicker-add-position');
...
// 添加输入文字按钮
const autoClickerAddInput = document.getElementById('nopic-autoclicker-add-input');
...
// 添加运行脚本按钮
const autoClickerAddScript = document.getElementById('nopic-autoclicker-add-script');
if (autoClickerAddScript) {
      ...
      if (val === 0) {
        showConfirmModal(
          ...
          () => {
            ...
            const confirmBox = document.querySelector('#nopic-confirm-modal .nopic-confirm-box');
  ...
  const autoClickerSaveBtn = document.getElementById('nopic-autoclicker-save-btn');
  if (autoClickerSaveBtn) {
    autoClickerSaveBtn.addEventListener('click', e => {
      ...
      const notifToggle = document.getElementById('nopic-autoclicker-notif-toggle');
      if (notifToggle) {
        autoClickerConfig.showNotification = notifToggle.classList.contains('on');
      }
      const autostartToggle = document.getElementById('nopic-autoclicker-autostart-toggle');
      if (autostartToggle) {
        autoClickerConfig.autoStartOnLoad = autostartToggle.classList.contains('on');
      }
  ...
  // 保存并执行按钮
const autoClickerSaveExecuteBtn = document.getElementById('nopic-autoclicker-save-execute-btn');
...
const autoClickerClearBtn = document.getElementById('nopic-autoclicker-clear-btn');
if (autoClickerClearBtn) {
  ...
  function scanAutoClickerFlows(scope) {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      try {
        ...
        const shortUrl = urlStr.length > 50 ? urlStr.substring(0, 50) + '...' : urlStr;
  ...
  // 清空本域名所有自动点击器流程
  const autoClickerClearDomainBtn = document.getElementById('nopic-autoclicker-clear-domain-btn');
  if (autoClickerClearDomainBtn) {
    autoClickerClearDomainBtn.addEventListener('click', e => {
      e.stopPropagation();
      const flows = scanAutoClickerFlows('domain');
      if (flows.length === 0) {
        showConfirmModal('无流程', '当前网站下没有找到任何自动点击器流程。', () => {});
        return;
      }
      const summary = flows.map((f, i) => `${i + 1}. ${f.url} (${f.steps.length}步)`).join('\n');
      showConfirmModal(
        '确认清空本域名流程？',
  ...
  // 清空全局所有自动点击器流程
  const autoClickerClearGlobalBtn = document.getElementById('nopic-autoclicker-clear-global-btn');
  if (autoClickerClearGlobalBtn) {
    autoClickerClearGlobalBtn.addEventListener('click', e => {
      e.stopPropagation();
      const flows = scanAutoClickerFlows('global');
      if (flows.length === 0) {
        showConfirmModal('无流程', '全局范围内没有找到任何自动点击器流程。', () => {});
        return;
      }
      const summary =
        flows.length <= 8
          ? flows.map((f, i) => `${i + 1}. ${f.url} (${f.steps.length}步)`).join('\n')
          : flows
              .slice(0, 8)
...
function triggerTextWaveEffect() {
  ...
  for (const el of allEls) {
    // 排除脚本UI
    if (el.closest('#nopic-widget') || el.closest('#nopic-menu') || 
        el.closest("[id^='nopic-']") || el.closest('#nopic-zoom-container') ||
        el.closest('#nopic-parade-overlay') || el.closest('#nopic-about-modal') ||
  ...
  // 按从上到下排序
  textLeaves.sort((a, b) => a.getBoundingClientRect().top - b.getBoundingClientRect().top);
...
privacyLockSubmenu.addEventListener('click', e => {
    ...
    if (e.target.closest('#nopic-privacy-log-btn') || e.target.closest('#nopic-privacy-log-panel')) {
        return;
    }
    ...
    if (privacyLockRuntime.isLocked && !e.target.closest('#nopic-privacy-log-btn')) {
        e.stopPropagation();
        e.preventDefault();
        return;
    }
    ...
    const selectBtn = e.target.closest('.nopic-privacy-select-btn[data-pl-key]');
    if (selectBtn) {
...
if (helpBtn) {
  helpBtn.addEventListener('click', function(e) {
    ...
    // 修改确认按钮文字
    const confirmBtn = document.querySelector('#nopic-confirm-modal .nopic-confirm-btn.danger');
    if (confirmBtn) {
...
if (customTextInput) customTextInput.value = appConfig.customText;
 ...
 privacyLockSubmenu.addEventListener('input', e => {
  ...
  if (target.dataset.plKey === 'password') {
    ...
    // 重新聚焦到输入框
    setTimeout(() => {
      const newTarget = document.querySelector(`input[data-pl-scope="${scope}"][data-pl-key="password"]`);
      if (newTarget) {
        newTarget.focus();
        newTarget.setSelectionRange(newTarget.value.length, newTarget.value.length);
      }
    }, 10);
...
document.getElementById('nopic-privacy-lock-now-btn').addEventListener('click', e => {
  e.stopPropagation();
  hidePrivacyLockSubmenu();
  showPrivacyLockOverlay();
});

document.getElementById('nopic-privacy-log-btn').addEventListener('click', function(e) {
    e.stopPropagation();
    e.preventDefault();
    privacyLogPanel.classList.add('active');
    logCurrentPage = 1;
    renderPrivacyLogs();
}, true); // 添加 true 表示在捕获阶段执行
...
if (forgotBtn) {
  ...
  if (logCurrentFilter !== 'all') {
    logs = logs.filter(log => {
      if (logCurrentFilter === 'unlock') {
        ...
        return log.action.includes('unlock') || 
               log.action === 'pin_skip' || 
      ...
      pageLogs.length === 0
        ...
        : pageLogs
            .map(log => {
              ...
              if (log.action.includes('unlock') && log.action !== 'forgot_unlock') {
                actionClass = 'unlock';
                actionText = '解锁';
  ...
  actionText = '跳过PIN解锁';
          ...
          <span class="nopic-privacy-log-status ${log.success ? 'success' : 'fail'}">${
            ...
            .join('');
    document.getElementById('nopic-log-page-info').textContent = `${logCurrentPage}/${totalPages}`;
    document.getElementById('nopic-log-prev-btn').disabled = logCurrentPage <= 1;
    document.getElementById('nopic-log-next-btn').disabled = logCurrentPage >= totalPages;
  }

  document.getElementById('nopic-privacy-log-close').addEventListener('click', e => {
    e.stopPropagation();
  ...
  document.getElementById('nopic-log-prev-btn').addEventListener('click', e => {
    e.stopPropagation();
    if (logCurrentPage > 1) {
      logCurrentPage--;
      renderPrivacyLogs();
  ...
  document.getElementById('nopic-log-next-btn').addEventListener('click', e => {
    e.stopPropagation();
    logCurrentPage++;
    renderPrivacyLogs();
  });
...
function hasStoredConfig(scope) {
  ...
  function enterTextReplaceEditMode(containerDiv, scope, idx, ruleData) {
    ...
    Array.from(containerDiv.children).forEach(c => (c.style.display = 'none'));
    containerDiv.appendChild(editRow);
  ...
  // 替换文字开关
  document.getElementById('nopic-replace-toggle').addEventListener('click', e => {
    e.stopPropagation();
  ...
  function finishRecording(combo) {
    ...
    if (combo) {
      ...
      // 恢复所有录制按钮状态
      quickTextSubmenu.querySelectorAll('.nopic-qt-record-btn').forEach(btn => {
        btn.textContent = '录制';
