  ...
  function dcRenderTable() {
    ...
    thead.querySelectorAll("th[data-col-id]").forEach(function (th) {
      th.addEventListener("dblclick", function (e) {
        ...
        if (!col) return;
        var newName = prompt("修改列名:", col.name);
...
`;
  ...
  document
    ...
    .addEventListener("click", function (e) {
      ...
      // 边界检测
      var menuRect = exportMenu.getBoundingClientRect();
      if (menuRect.right > window.innerWidth - 10) {
        exportMenu.style.left = window.innerWidth - menuRect.width - 10 + "px";
      }
      if (menuRect.bottom > window.innerHeight - 10) {
        exportMenu.style.top = rect.top - menuRect.height - 4 + "px";
      }
    });
  ...
  function exportCSV() {
    ...
    for (var rowIdx = 0; rowIdx < maxRows; rowIdx++) {
      var rowData = dcState.columns.map(function (col) {
        ...
        // text_with_link / text_with_link_image：提取文本，并附带图片URL
        if (
          col.type === "text_with_link" ||
          col.type === "text_with_link_image"
        ) {
    ...
    link.download =
      "采集数据_" + new Date().toISOString().slice(0, 10) + ".csv";
  ...
  function exportHTML() {
    ...
    for (var rowIdx = 0; rowIdx < maxRows; rowIdx++) {
      var rowData = dcState.columns.map(function (col) {
        ...
        // text_with_link / text_with_link_image
        if (
          col.type === "text_with_link" ||
          col.type === "text_with_link_image"
        ) {
            ...
            if (parsed && typeof parsed === "object") {
              ...
              if (parsed.image) {
                parts.push(
                  '<img src="' +
                    dcEscapeHtml(parsed.image) +
                    '" style="max-height:60px;max-width:120px;border-radius:4px;object-fit:contain;">',
                );
              }
              if (parsed.link) {
                parts.push(
                  '<a href="' +
                    ...
                    '" target="_blank">' +
                    dcEscapeHtml(parsed.text || parsed.link) +
                    "</a>",
                );
              } else if (parsed.text) {
    ...
    link.href = URL.createObjectURL(blob);
    link.download =
      "采集数据_" + new Date().toISOString().slice(0, 10) + ".html";
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(link.href);
